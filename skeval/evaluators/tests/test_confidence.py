...
class TestConfidenceThresholdEvaluator(unittest.TestCase):
    ...
    def test_estimate_with_dict_scorer(self):
        ...
        scorers = {'accuracy': accuracy_score, 'precision': lambda y_t, y_p: precision_score(y_t, y_p, zero_division=0)}
    ...
    def test_invalid_scorer_type_raises_error(self):
        ...
        evaluator = ConfidenceThresholdEvaluator(self.classifier, scorer="not_a_callable")
        with self.assertRaisesRegex(ValueError, "'scorer' must be a callable or a dict of callables."):
            evaluator.estimate(self.X_test)
    ...
    def test_limit_to_top_class_false_raises_error(self):
        ...
        """
        evaluator = ConfidenceThresholdEvaluator(self.classifier, limit_to_top_class=False)
